...
운동 인증 커뮤니티 서비스 SEVEN의 Full-stack 프로젝트입니다.

## ✨ Live Demo

*   **Frontend:** [https://nb-project-seven-fe.vercel.app/](https://nb-project-seven-fe.vercel.app/)
*   **Backend API:** [https://nb04-seven-team5.onrender.com](https://nb04-seven-team5.onrender.com)

---

## 🚀 로컬에서 실행하기

이 프로젝트는 백엔드와 프론트엔드로 구성된 모노레포(monorepo) 구조를 가집니다.

### Backend

Node.js, Express, Prisma로 구축된 백엔드 서버입니다.
...
3.  **데이터베이스 설정**
    Prisma를 사용하여 데이터베이스 스키마를 적용해야 합니다.
    ```bash
    npx prisma migrate dev
    npx prisma generate
    ```
    또한, 프로젝트 루트에 `.env` 파일을 생성하고 `DATABASE_URL`을 설정해야 합니다.
    ```env
    # .env 예시
...
4.  **실행**
    ...
    npm run dev
    ```
    > 백엔드 서버는 `http://localhost:3001`에서 실행됩니다. (프론트엔드와의 포트 충돌을 피하기 위함)

---

### Frontend

Next.js, TypeScript로 구축된 프론트엔드 애플리케이션입니다.

1.  **위치:** `/project-seven-fe-main`

2.  **의존성 설치**
    ```bash
    cd project-seven-fe-main
    npm install
    ```

3.  **환경 변수 설정**
    `/project-seven-fe-main` 폴더에 `.env` 파일을 생성하고 로컬 백엔드 API 주소를 설정합니다.
    ```env
    # .env 예시
