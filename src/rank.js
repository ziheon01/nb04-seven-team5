...
객체로 할당시 {COUNT, DURATION} - { COUNT : 'count', DURATION : 'duration' }
*/
const [COUNT, DURATION] = ['count', 'duration']
...
async function getRanks(req, res, next) {
    ...
    try {
        ...
        //잘못된 쿼리 요청에 대한 응답
        return res.status(400).json({
            error: "ranking 쿼리 파라미터는 'count' 또는 'duration'이어야 합니다."
        });
...
app.get(`/groups/:groupId/participants`, getRanks)